...
export const listener = FlatfileListener.create((listener) => {
  ...
  listener.use(
    recordHook("frontend", (record) => {

      if (record.get('Borrower First/Middle Name')) {
        const name = record.get("Borrower First/Middle Name") as string;
        const validName = /^[\D\s]+$/g
        if (!validName.test(name)) {
          record.addError("Borrower First/Middle Name", "Names cannot contain numbers");
        }
      }

      if (record.get('Borrower Last Name/Suffix')) {
        const name = record.get("Borrower Last Name/Suffix") as string;
        const validName = /^[\D\s]+$/g
        if (!validName.test(name)) {
          record.addError("Borrower Last Name/Suffix", "Names cannot contain numbers");
        }
      }

      if (record.get('Co-Borrower First/Middle Name')) {
        const name = record.get("Co-Borrower First/Middle Name") as string;
        const validName = /^[\D\s]+$/g
        if (!validName.test(name)) {
          record.addError("Co-Borrower First/Middle Name", "Names cannot contain numbers");
        }
      }

      if (record.get('Co-Borrower Last Name/Suffix')) {
        const name = record.get("Co-Borrower Last Name/Suffix") as string;
        const validName = /^[\D\s]+$/g
        if (!validName.test(name)) {
          record.addError("Co-Borrower Last Name/Suffix", "Names cannot contain numbers");
        }
      }
